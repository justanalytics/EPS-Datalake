...
where 
year(document_date) in (YEAR(GETDATE()),YEAR(GETDATE())-1,YEAR(GETDATE())-2) and (case when vessel.include_in_drc = 1 and (year(vessel.sold_date) in (YEAR(GETDATE()),YEAR(GETDATE())-1,YEAR(GETDATE())-2) or vessel.sold_date is null) and vessel.fleet is not null then 1 else 0 end) = 1;
